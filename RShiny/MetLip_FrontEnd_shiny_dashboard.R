...
server <- function(input, output, session) {
  ...
  observeEvent(input$generate_plate_btn, {
    ...
    if (length(rv$parsed_cols) > 0) {
      ...
      # confirm the target join key exists in plate_loading before joining;
      # a mismatched key would otherwise produce all-NA covariate columns,
      # which behave like no grouping at all
      target_key <- "Submitted_Sample_ID"
      if (!target_key %in% colnames(processed$plate_loading)) {
        showNotification(
          paste0("Join key '", target_key, "' not found in plate_loading. ",
                 "Available columns: ",
        ...
        if (nrow(processed$plate_loading) != before)
          showNotification(
            paste0("Row count changed after join (", before, " -> ",
                   ...
                   "). Covariates are fanning out; grouping may be unreliable."),
            type = "warning", duration = 10)
        
        # verify covariates actually populated after the join
        na_all <- vapply(rv$parsed_cols, function(c)
          all(is.na(processed$plate_loading[[c]])), logical(1))
        if (any(na_all))
          showNotification(
            paste("These covariates are all NA after join and will not group:",
