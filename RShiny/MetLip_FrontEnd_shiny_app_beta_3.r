...
# ── Local shared secret (Shiny ↔ Plumber) ─────────────────────────────────────
...
# Helper: detect submitted sample ID column
              ...
              fluidRow(box(
                ...
                shinyjs::hidden(div(id = "upload_panel",
                                    fileInput("plate_file_input", "Upload Acquired Samples Excel:",
                                              accept = c(".xlsx", ".xls")),
                                    tags$p(tags$strong("Attention:"),
                                           " keep the ", tags$code("Notes"),
                                           " column in your parsed file.",
                                           style = "color: #355e3b; font-size: 13px; margin-top: -8px;")
                )),
...
server <- function(input, output, session) {
  ...
  rv <- reactiveValues(
    ...
    df_source        = NULL,
    plate_id_col     = NULL,
    parsed_cols      = character(),   # extra columns detected in the source file
    df_validated     = NULL,
    plate_results    = NULL,
  ...
  observeEvent(input$search_btn, {
    ...
    res <- tryCatch(
      ...
      error = function(e) {
        showNotification(paste("Cannot reach Plumber:", e$message, "Call Ian"), type = "error"); NULL
      }
    )
  ...
  observeEvent(input$upload_btn, {
    ...
    res <- tryCatch(
      POST("http://localhost:8000/pcs/upload",
           ...
           body = charToRaw(upload_payload), encode = "raw"),
      error = function(e) {
        output$upload_status <- renderUI(
          tags$p(paste("API unreachable:", e$message, ". Call Ian"), class = "status-msg error"))
  ...
  observeEvent(input$load_source_btn, {
    ...
    # Detect parsed/extra columns — anything beyond the standard TAS columns.
    # These are candidates for randomization grouping and are carried into the
    # plate meta data files.
  ...
  observeEvent(input$generate_plate_btn, {
    ...
    if (length(rv$parsed_cols) > 0) {
      ...
      # plate_loading uses Submitted_Sample_ID; map the join key accordingly
    ...
    # Detect unique MS methods for LC method inputs
  ...
  # Render one LC Method input per detected MS method
  ...
  output$group_cols_ui <- renderUI({
    cols <- rv$parsed_cols
    if (length(cols) == 0)
      return(tags$em("No extra columns detected — a simple shuffle will be used."))
  ...
  # Generate Sequence — pass LC method mapping and QC/blank position overrides
  observeEvent(input$generate_seq_btn, {
    ...
    # Build LC method lookup from the per-MS-method inputs on this tab
    ms <- rv$ms_methods_detected
    ...
    # Warn if any LC method left empty
    if (any(lc_vals == "")) {
